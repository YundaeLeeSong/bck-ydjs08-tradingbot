...
*   **Single Responsibility per File:** 
    ...
    *   `analysis_service.py` & `matplotlib_service.py`: Strict decoupling of pure mathematical analysis calculations from graph plotting logic.
...
*   **Import Placement & Cleanup:** ALL imports MUST be declared at the top of the file. Do NOT use nested imports (imports inside functions or methods) unless strictly necessary to prevent circular dependencies. Regularly clean up and remove any unused imports.
*   **Always resolve warnings:** Address any warnings generated by Python packages appropriately using idiomatic features.

## 4. Managing TODOs.md
When updating `TODOs.md`, adhere to the following rules:
*   **Format:** Keep instructions short, formatted as a numbered list, and easy to understand.
*   **Precision:** Be precise and direct about the technical requirements without unnecessary verbosity.
*   **Maintenance:** Overwrite or clear out completed tasks. The file should only reflect the current, pending workload.